...
class EBS9Handler:
    ...
    def extract(
        ...
        # Layer 1 — EBS9 ekstrak teks dari MRI stego
        if orig_txt is None:
            raise ValueError("EBS9 membutuhkan teks asli untuk proses ekstraksi.")

        t1_start = time.perf_counter()
        result_l1 = ebs9.extract(extracted_mri_img, orig_txt)
        time_layer1 = round(time.perf_counter() - t1_start, 6)
