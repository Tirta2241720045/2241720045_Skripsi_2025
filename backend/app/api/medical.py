...
def _parse_method_from_stego(stego_photo_path: str) -> Optional[str]:
    """Baca method dari nama file stego"""
    try:
        basename = os.path.splitext(os.path.basename(stego_photo_path))[0]
        parts = basename.split('_', 3)
        # Format: stego_{patient_id}_{timestamp}_{method}.png
        if len(parts) >= 4:
            method = parts[3]
