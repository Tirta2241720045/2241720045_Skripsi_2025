...
type MobileTab = 'statistics' | 'users' | 'logs';
...
function getRoleIconUrl(role: string): string {
  ...
  switch (role) {
    ...
    case 'doctor': 
      return { 
        ...
        border: 'var(--role-doctor-border)', 
        badge: 'var(--role-doctor-badge)', 
        label: 'Doctor', 
        icon: '🩺', 
        color: '#22c55e' 
      };
...
const DashboardAdmin = () => {
  ...
  // Mobile states
  const [mobileTab, setMobileTab] = useState<MobileTab>('statistics');
  ...
  const chartOption = {
    ...
    plugins: { 
      legend: { 
        ...
        labels: { font: { size: 10, family: 'DM Sans' }, boxWidth: 8, padding: 8 } 
      } 
    },
  ...
  const barLineOption = {
    ...
    scales: {
      x: { 
        ticks: { font: { size: 9, family: 'DM Sans' } }, 
        grid: { display: false } 
      },
      y: { 
        ticks: { font: { size: 9, family: 'DM Sans' } }, 
  ...
  const getActivityByUser = () => {
    ...
    allLogs.forEach(log => { 
      map[log.user_id] = (map[log.user_id] || 0) + 1; 
    });
  ...
  const getPeakPerformance = () => {
    ...
    base.forEach(log => { 
      hours[new Date(log.timestamp).getHours()]++; 
    });
    ...
    base.forEach(log => {
      ...
      if (user) {
        if (!userActivity[log.user_id]) {
          userActivity[log.user_id] = { name: user.full_name, count: 0, role: user.role };
        }
  ...
  const generateInsights = (): string[] => {
    ...
    const failedLogins = allLogs.filter(l => 
      l.action.toUpperCase().includes('LOGIN_FAILED') || 
      l.action.toUpperCase().includes('LOGIN FAILED')
    );
  ...
  const pieData = {
    ...
    datasets: [{ 
      data: [roleCounts.admin, roleCounts.doctor, roleCounts.staff], 
        ...
        >
          ...
          {isSelected && !isEditing && !isCreating && (
            <button 
              ...
              onClick={(e) => { e.stopPropagation(); showConfirmDelete(u, () => handleDeleteUser(u)); }} 
              title="Delete"
            >
              🗑
            </button>
  ...
  const renderMobileUserList = () => {
    return (
      <div className="mobile-user-list-vertical">
        {sortedUsers.map(u => {
          const isSelected = selectedUser?.user_id === u.user_id;
          const accent = getRoleAccent(u.role);
          return (
            <div
              key={u.user_id}
              className={`mobile-user-row mobile-user-row-${u.role} ${isSelected ? 'selected' : ''}`}
              onClick={() => { if (!isEditing) handleSelectUser(u); }}
            >
              <div className="mobile-user-row-avatar">
                <img
                  src={getRoleIconUrl(u.role)}
                  alt={u.role}
                  className="mobile-user-row-avatar-img"
                  onError={(e) => {
                    e.currentTarget.style.display = 'none';
                    const parent = e.currentTarget.parentElement;
                    if (parent) {
              ...
              <div className="mobile-user-row-info">
                <span className="mobile-user-row-name">{u.full_name}</span>
                <span className={`mobile-user-row-role mobile-user-row-role-${u.role}`}>
                  {accent.icon} {accent.label}
                </span>
              </div>
              {isSelected && !isEditing && !isCreating && (
                <div className="mobile-user-row-actions">
                  <button
                    className="mobile-user-row-btn mobile-user-row-btn-edit"
                    onClick={(e) => { e.stopPropagation(); setIsEditing(true); }}
                  >
                    ✎
                  </button>
                  <button
                    className="mobile-user-row-btn mobile-user-row-btn-delete"
                    onClick={(e) => { e.stopPropagation(); showConfirmDelete(u, () => handleDeleteUser(u)); }}
                  >
                    🗑
                  </button>
              ...
              {isSelected && isEditing && !isCreating && (
                <div className="mobile-user-row-actions">
                  <button
                    className="mobile-user-row-btn mobile-user-row-btn-save"
                    onClick={(e) => { e.stopPropagation(); handleSaveEdit(); }}
                  >
                    ✓
                  </button>
                  <button
                    className="mobile-user-row-btn mobile-user-row-btn-cancel"
                    onClick={(e) => {
                      e.stopPropagation();
                      setIsEditing(false);
                      if (selectedUser) {
        ...
        <div
          className="mobile-user-row mobile-user-row-add"
          onClick={() => { 
            setIsCreating(true); 
            setSelectedUser(null); 
            setIsEditing(false); 
        ...
        >
          <div className="mobile-user-row-avatar mobile-user-row-add-avatar">
            <span className="mobile-user-row-add-icon">+</span>
          </div>
          <div className="mobile-user-row-info">
            <span className="mobile-user-row-name add-text">Add Member</span>
          </div>
        </div>
      </div>
    );
          ...
          <div className="detail-content">
            <div className="detail-avatar-section">
              <div className="detail-avatar-wrapper">
                {selectedUser ? (
                  ...
                  <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: '32px' }}>
                    👤
                  </div>
                )}
              </div>
            ...
            <div className="detail-form compact">
              <div className="form-group compact">
                ...
                <input 
                  type="text" 
                  value={selectedUser ? editFormData.full_name : ''} 
                  onChange={e => setEditFormData({ ...editFormData, full_name: e.target.value })} 
                  disabled={!isEditing || !selectedUser} 
                  placeholder={selectedUser ? '' : 'Select a staff'} 
                />
              </div>
              <div className="form-group compact">
                <label>Username</label>
                <input 
                  type="text" 
                  value={selectedUser ? editFormData.username : ''} 
              ...
              <div className="form-group compact">
                <label>Role</label>
                <select 
                  value={selectedUser ? editFormData.role : ''} 
              ...
              <div className="form-group compact password-change-section">
                ...
                {!isChangingPassword ? (
                  <button 
                    type="button" 
                    className="btn-change-password-full" 
            ...
            <div className="logs-list">
              {filteredLogs.map((log) => (
                <div key={log.log_id} className="log-item">
                  <div className={`log-avatar ${getLogAvatarClass(log.action)}`}>
                    {getLogAvatarLetter(log.action)}
                  </div>
  ...
  const renderMobileChartCard = (chartType: ChartType) => {
    return (
      <div className="mobile-chart-card" key={chartType}>
        <div className="mobile-chart-card-header">
          <span className="mobile-chart-card-icon">{CHART_ICONS[chartType]}</span>
          <span className="mobile-chart-card-title">{CHART_LABELS[chartType]}</span>
        </div>
        <div className="mobile-chart-card-body">
          {chartType === 'role' && (
            <>
              <div className="mobile-chart-area">
                <Pie data={pieData} options={chartOption} />
              </div>
              <div className="mobile-legend-area">
                {[
                  { label: 'Administrator', count: roleCounts.admin, color: '#ef4444' },
                  { label: 'Doctor', count: roleCounts.doctor, color: '#22c55e' },
                  { label: 'Medical Staff', count: roleCounts.staff, color: '#f59e0b' },
          ...
          {chartType === 'overview' && (
            <>
              ...
              <div className="mobile-chart-full">
                <Bar
                  data={{
                    labels: activityByDay.labels,
                    datasets: [{
  ...
  const renderMobileStatistics = () => {
    return (
      <div className="mobile-statistics-container">
        <div className="mobile-statistics-header-card">
          <div className="mobile-statistics-header-left">
            <span className="mobile-statistics-header-icon">📊</span>
            <h3>Statistics</h3>
          </div>
          <div className="mobile-statistics-header-right">
            <span className="mobile-statistics-viewing">
              {selectedUser ? selectedUser.full_name : 'All Users'}
            </span>
            <button onClick={handleExportToPDF} className="btn-stat-export-mobile" title="Export PDF">
              <span>↓</span> PDF
            </button>
          </div>
        </div>
        <div className="mobile-charts-grid">
          {CHARTS.map(chartType => renderMobileChartCard(chartType))}
        </div>
      </div>
    );
  };
      ...
      case 'users':
        return (
          <div className="mobile-tab-content">
            ...
            <section className="card mobile-users-card">
              <div className="card-header">
                <h3>Team Members</h3>
              </div>
        ...
        {isMobile ? (
          <div className="mobile-layout">
            ...
            <nav className="bottom-navbar">
              <button 
                className={`bottom-nav-item ${mobileTab === 'statistics' ? 'active' : ''}`}
              ...
              <button 
                className={`bottom-nav-item ${mobileTab === 'users' ? 'active' : ''}`}
              ...
              <button 
                className={`bottom-nav-item ${mobileTab === 'logs' ? 'active' : ''}`}
                onClick={() => setMobileTab('logs')}
              >
                <span className="bottom-nav-icon">📋</span>
                <span className="bottom-nav-label">Log Activity</span>
              </button>
            </nav>
          </div>
        ) : (
